...
function Sharp(config) {
    ...
    // Had to move the logic down because the object was being contaminated after it was used
    ...
    _.forEach(cfg, function(conf){
        ...
        _.forEach(imageFiles, function (file) {
            _.forEach(conf.tasks, function(task){
                // Recreated every time because the object is a reference and it got contaminated each time.
                task = createSharpPipeline(task);

                sharp(file.src).pipe(task.task).toBuffer().then(function (data) {
...
var fetchFiles = function(src, dest, imageExt) {
    ...
    var files = glob.sync(src, {nodir: true, ignore: dest}).filter(anymatch(buildExtPattern(imageExt)));

    var buildFileObj = function (file) {
        return {src: file, ext: path.extname(file), dest: destFile(file, src, dest)};
    };
...
var writeFile = function (file, task, contents, callback) {
    ...
    mkdirpPromise(dir).then(function() {
        ...
        var dest = file.dest;
        // Replace extension (must be done, case when `toFormat` is used but no `rename`)
        ...
        if(task.rename) {
            ...
            // Do the magic rename
            dest = path.join(dir, processName(task.rename, {base: base, ext: ext}));
        }
...
var replaceExt = function (pathStr, ext) {
    return path.join(
        ...
        path.basename(pathStr, path.extname(pathStr)) + "." + removeExtDot(ext));
};
