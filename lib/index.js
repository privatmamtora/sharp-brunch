...
Sharp.prototype.validateConfig = function(callback) {
    ...
    // ': options object must be plain object (created with `{}` literal) ');
    } //else if ( this.options.options.resize === undefined
    // && this.options.options.extract === undefined ) {
    //    return console.error(PLUGIN_NAME +
    // ': Please specify an extract or resize property in your options object');
    //} else if ( this.options.options.resize
    // && Array.isArray( this.options.options.resize ) === false ) {
    //    return console.error(PLUGIN_NAME +
    // ': options.resize must be array');
    //}
